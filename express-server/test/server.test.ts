...
describe("Test POST /event/:eventId/assign/:userId", () => {
  ...
  test("Test for a user that doesn't exist", async () => {
    ...
    await axios.post(`http://localhost:3001/event/1/assign/${nonExistentUserId}`, {}, { headers })
      .catch((err) => {
        expect(err.response.status).toBe(404);
      });
  });
  ...
  test("Test for an event that is already assigned to a user", async () => {
    await axios.post(`http://localhost:3001/event/1/assign/1`, {}, { headers })
      .catch((err) => {
...
describe("Test GET /event/:eventId", () => {
  test("Test normally", async () => {
    ...
    const createRes = await axios.post("http://localhost:3001/event", event, { headers: headers });
    const eventId = createRes.data.eventId;

    // retrieve the event
    const getRes = await axios.get(`http://localhost:3001/event/${eventId}`, { headers: headers });
    ...
    await axios.delete(`http://localhost:3001/event/${eventId}`, { headers })
      .then((response) => {
  ...
  test("Test for an event that doesn't exist", async () => {
    await axios.get("http://localhost:3001/event/zzz", { headers })
      .catch((err) => {
        expect(err.response.status).toBe(404);
      });
  });
...
describe("Test POST /event and DELETE /event/:eventId", () => {
  ...
  test("Test normally", async () => {
    ...
    await axios.post("http://localhost:3001/event", event, { headers })
      .then((response) => {
  ...
  test("Delete an event that doesn't exist", async () => {
    await axios.delete(`http://localhost:3001/event/zzz`, { headers })
      .catch((err) => {
        expect(err.response.status).toBe(404);
      });
  });
...
describe("Test GET /user/:userId/events", () => {
  ...
  test("Test normally", async () => {
    ...
    const userResponse = await axios.post("http://localhost:3001/user", user, {
      headers: headers
    })
    ...
    const endDate = new Date(new Date(startDate.getTime() + 60 * 60 * 1000).setMilliseconds(0)) // add 1 hour to startDate
    const event1: EventType = {
      title: "Test event 1",
